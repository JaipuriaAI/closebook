...
mkdir -p ~/.claude/skills/finance && \
  curl -o ~/.claude/skills/finance/SKILL.md \
  https://finance-two.vercel.app/SKILL.md
```
...
> *Process this statement with the finance skill: path-to-statement.pdf*

On first run the skill walks you through a one-time config wizard (buckets, merchant rules, markup %, tax %, bill-to blocks, exclusion list) and saves it to `finance-config.yaml` in the working directory.
...
The live deployment is at https://finance-two.vercel.app.

To attach a custom domain (e.g. `finance.yourdomain.com`):
...
| **Install URL** | `https://finance-two.vercel.app/SKILL.md` |
| **Author** | Rehearsal AI |
| **License** | MIT |
...
All of this lives in `finance-config.yaml` in your working directory. Nothing is hard-coded into the skill. Nothing leaves your machine.
